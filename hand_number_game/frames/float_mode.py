...
class FloatMode(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.answered_label = tk.Label(
            self, text=f"Jumlah Skor : 0", font=REGULAR_FONT)
        self.answered_label.pack()

        self.media = Media

        self.highestScore = self.media.readHighestScore("float")
    ...
    def pop_up(self):
        ...
        pop_label = tk.Label(
            pop, text="Kamu kurang cepat, coba lagi", font=REGULAR_FONT)
        pop_label.pack(pady=10)

        back_button = tk.Button(
            pop, text="Kembali", font=REGULAR_FONT, command=lambda: pop.destroy())
        back_button.pack()
        ...
        while self.activeCam:
            ...
            if x >= 600:
                ...
                if self.answered > self.highestScore:
                    ...
                    self.media.updateHighestScore(self.answered, "float")
            ...
            cv2.putText(
                img, number, (xnum,
                              ynum), cv2.FONT_HERSHEY_PLAIN, 4, (255, 255, 0), 5
            )

            # Remaining lifes
            cv2.putText(
                img, f"Nyawa : {self.lifes}", (
                    20, 70), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 0), 2
            )
